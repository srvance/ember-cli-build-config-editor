...
function keyRepresentation(property) {
  var tokens = esprima.tokenize(property);
  if (tokens && tokens.length === 1 && tokens[0].type === 'Identifier') {
    return builders.identifier(property);
  }
  return builders.literal(property);
}
